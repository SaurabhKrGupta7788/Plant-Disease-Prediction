...
3. Submit the image.
4. The system will asynchronously pass the image to the PyTorch backend, replacing the underscores in the class name with a readable format and displaying the confidence score.

## Key Design Decisions

1. **In-Memory Image Processing**: To avoid disk I/O bottlenecks and temporary file clutter, uploaded images are passed as raw bytes `io.BytesIO()` directly from the Django `request.FILES` object into the PIL processing pipeline.
